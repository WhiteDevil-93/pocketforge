              ...
              {ready && data && (
                <Tabs defaultValue="all" className="pt-3">
                  ...
                  <TabsContent value="winning-moves" className="pt-2">
                    {data.derivedMetrics === 'derived' && data.winningMoves && data.winningMoves.length > 0 ? (
                      <div className="space-y-2">
                        {data.winningMoves.map((move) => (
                      ...
                      <NotEnoughData
                        title="Not enough data"
                        description="Smogon monthly dumps expose move usage only — per-move win/loss correlation isn't available, so winning moves can't be derived without fabricating numbers."
                      />
                    )}
                  </TabsContent>

                  <TabsContent value="losing-moves" className="pt-2">
                    {data.derivedMetrics === 'derived' && data.losingMoves && data.losingMoves.length > 0 ? (
                      <div className="space-y-2">
                        {data.losingMoves.map((move) => (
                          <MoveCard key={move.rank} move={move} />
                        ))}
                      </div>
                    ) : (
                      <NotEnoughData
                        title="Not enough data"
                        description="Smogon monthly dumps expose move usage only — per-move win/loss correlation isn't available, so losing moves can't be derived without fabricating numbers."
                      />
                    )}
                  </TabsContent>
                </Tabs>
