...
// data' state. Seed lookups are synchronous and offline; the one hydration
// check stubs out PokéAPI with 404s so it exercises the local-data fallback
// deterministically without network access.
// ============================================================================

import { getAvailableBattleFormats, getPokemonBattleData, getPokemonBattleDataSeed } from '@/lib/data';
import {
...
if (seedResult) {
  ...
  check('stat alignment / stat points tabs have data rows', () => {
    ...
    assert(typeof spread.evs.hp === 'number', 'expected numeric EV allocation');
  });
...
check('missing format resolves to null', () => {
  const missing = getPokemonBattleDataSeed(topSpecies, 'gen9notaformat', 1630);
  assertEqual(missing, null);
});

// Offline hydration: stubbing PokéAPI with 404s forces the local-data
// fallback, so move cards still get type/power/accuracy/PP metadata.
async function checkHydration() {
  try {
    assert(vgcFormat !== undefined, 'expected a VGC format in the snapshot');
    const data = await getPokemonBattleData(topSpecies, vgcFormat.format, {
      fetchOptions: {
        fetchImpl: async () => new Response(null, { status: 404 }),
      },
    });
    assert(data !== null, 'expected hydrated battle data');
    assert(data.moves.length > 0, 'expected hydrated moves');
    const topMove = data.moves[0];
    assert(topMove.type !== null, 'expected a move type from local data');
    assert(topMove.power !== null, 'expected move power from local data');
    assert(topMove.pp !== null, 'expected move PP from local data');
    assertEqual(data.derivedMetrics, 'not-enough-data');
    assertEqual(data.winningMoves, undefined);
    console.log('  ok — getPokemonBattleData hydrates move metadata offline (local fallback)');
  } catch (error) {
    failures.push('getPokemonBattleData hydrates move metadata offline');
    console.error(
      '  FAIL — getPokemonBattleData hydrates move metadata offline:',
