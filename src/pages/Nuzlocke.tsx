...
function getBossCategory(boss: { name?: string; location?: string; id?: string }): 'gym' | 'elite4' | 'champion' | 'rival' | 'evil' | 'titan' | 'other' {
...
function RunCard({ run, isActive, onSelect, onDelete }: { run: NuzlockeRun; isActive: boolean; onSelect: () => void; onDelete: () => void }) {
...
// ---- VS Comparison Panel ----
function VSComparison({ boss, teamEncounters, onClose }: { boss: BossData; teamEncounters: NuzlockeEncounter[]; onClose: () => void }) {
      ...
      <div className="bg-bg-secondary rounded-t-3xl max-h-[80vh] overflow-y-auto p-5 pb-28" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="mb-4">
          ...
          <div className="grid grid-cols-3 gap-2">
            {boss.team.map((p: BossPokemonEntry, i: number) => {
...
interface BossData {
  ...
  type: string;
  levelCap: number;
  team: BossPokemonEntry[];
}

// ---- Boss Card ----
function BossCard({ boss, teamTypes, onVS }: { boss: BossData; teamTypes: string[]; onVS: (b: BossData) => void }) {
  const [open, setOpen] = useState(false);
...
export default function Nuzlocke() {
  ...
  const [vsBoss, setVsBoss] = useState<BossData | null>(null);
  ...
  const handleTeraRoll = useCallback((routeId: string) => {
    if (!currentRunId) return;
