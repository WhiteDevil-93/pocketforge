...
// hydrates it with PokéAPI metadata via the pokeapi.ts layer and exposes
// getPokemonBattleData(pokemon, format, cutoff?) → BattleData.
//
// The snapshot is imported statically here; consumers that want to keep it out
// of the initial bundle should dynamic-import this module.
//
// NO numbers are ever computed at runtime — every percentage/rank is read
// straight from the parsed dump (rounding happens only in the UI layer).
// ============================================================================
...
import { hydrateRankedAbility, hydrateRankedItem, hydrateRankedMove } from './pokeapi';
import type { PokeApiFetchOptions } from './pokeapi';
import type {
  BattleData,
  BattleFormatId,
  EvSpread,
...
export interface BattleDataOptions {
  ...
  fetchOptions?: PokeApiFetchOptions;
}
...
function cutoffSnapshotFor(formatSnap: SmogonFormatSnapshot, requested: RatingCutoff | undefined): SmogonCutoffSnapshot {
  const requestedKey = requested !== undefined ? String(requested) : '';
  return formatSnap.cutoffs[requestedKey] ?? formatSnap.cutoffs[String(formatSnap.defaultCutoff)];
}
...
function snapshotCutoffFor(format: BattleFormatId, ratingCutoff: RatingCutoff | undefined): SmogonCutoffSnapshot | null {
  ...
  const cutoffSnap = cutoffSnapshotFor(formatSnap, ratingCutoff);
...
export async function getPokemonBattleData(
  ...
  return {
    ...
    matchupMoves: computeMatchupMoves(),
    lastUpdated: cutoff.lastUpdated,
  };
}

/**
 * Derived 'Winning Moves' / 'Losing Moves' lists for a Pokémon.
 *
 * Smogon monthly dumps contain move *usage* but no per-move win/loss
 * correlation, so no honest heuristic can rank moves by matchup outcome. This
 * returns an explicitly-flagged empty result ('not enough data') rather than
 * fabricating numbers; `method` documents why.
 */
export function getMatchupMoves(
  pokemon: string,
  format: BattleFormatId,
  options: BattleDataOptions = {},
): MatchupMoves | null {
...
async function hydrateRankedRows<T>(
  seeds: Array<{ rank: number; name: string; usagePercent: number }>,
  hydrate: (seed: { rank: number; name: string; usagePercent: number }) => Promise<T | null>,
): Promise<T[]> {
...
function toRankedMatchup(seed: SmogonCheckSeed): RankedMatchup {
  return {
    ...
    koChance: seed.koChance,
    koChanceText: seed.koChanceText,
    switchInChance: seed.switchInChance,
    spriteUrl: getSpriteUrl(seed.opponent),
  };
}

const MATCHUP_MOVES_UNAVAILABLE = 'Smogon monthly dumps expose move usage only — per-move win/loss correlation is not available, so winning/losing move lists are flagged "not enough data" rather than fabricated.';

function computeMatchupMoves(): MatchupMoves {
  return {
    available: false,
    method: MATCHUP_MOVES_UNAVAILABLE,
    winningMoves: [],
    losingMoves: [],
  };
}
