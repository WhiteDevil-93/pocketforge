...
that hands an agent a Google Colab notebook as a remote runtime — usually with a free GPU,
though Google guarantees neither free-tier access nor which accelerator you get (a T4 is
the common case, not a promise). For PocketForge that covers the parts of the on-device LLM
work that need a GPU but not a long-lived machine, against the **Gemma 4 E4B** base:
template round-trip checks, GGUF quantisation, one-off experiments.
...
Using a specific notebook instead of the scratch one would mean opening it yourself and
appending the same `#mcpProxyToken=…&mcpProxyPort=…` fragment the tool used — but as of this
writing (pinned commit in [`.mcp.json`](../.mcp.json)), upstream only logs the port, not the
token, so there is no supported way to recover that fragment after the fact. Stick to the
scratch notebook `open_colab_browser_connection` opens until upstream exposes one.
...
| Short QLoRA / LoRA run on Gemma 4 E4B | Tight | E4B is roughly double E2B; if the free tier assigns a T4 (16 GB, not guaranteed — see above), it leaves little headroom, and Turing has no bf16. Checkpoint to Drive every N steps — a reclaimed session loses local disk |
| Multi-hour full fine-tune | Bad | Use the GCE T4 recipe from [`drive-to-gcp-model-transfer.md`](./drive-to-gcp-model-transfer.md) — see that doc for the current cost estimate |
